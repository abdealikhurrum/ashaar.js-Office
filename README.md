...
- **Jameel Noori Nastaleeq Kasheeda** — offered as a font-mode option but not bundled; the file is private/user-supplied and there is no `@font-face` block for it. It is **render-only**: Word renders it if the reader has it installed, but kashida auto-fill is not available (Gate G, 2026-07-10, found Word only slants italic runs instead of swapping in kasheeda glyphs — see `src/taskpane/taskpane.css`).
...
- **Word documents render with the document's own installed fonts.** The taskpane only writes a font *name* into the OOXML (`<w:rFonts w:cs="…">`); Word does not embed the font file. This means each font mode has a different reader-side requirement, shown as a note under the Font picker (`#font-install-note` in `src/taskpane/taskpane.html`, wired by `updateFontNote()` in `src/taskpane/taskpane.js`):
  ...
  - **Jameel Noori Kasheeda** — note shown; the reader must have "Jameel Noori Nastaleeq" installed to see this correctly. Render-only: kashida auto-fill is not available in Word for this font (Gate G FAILED, 2026-07-10 — Word only slants italic runs, it does not swap in kasheeda glyphs), so it fills justified lines by spacing like the other whitespace-mechanism fonts.
...
- **Jameel Noori Nastaleeq Kasheeda is private and is never deployed publicly.** The add-in is served as a static site from GitHub Pages, so anything committed to `assets/fonts/` is world-readable at a public URL. The Jameel font file's license does not permit that kind of public rehosting, so:
  ...
  - The font mode still appears in the picker with no `@font-face` rule in `taskpane.css`; it is meant to be dropped into `assets/fonts/` locally (or loaded via the in-app "Load a font from your computer" custom-font flow) for development and for testers who already hold a licensed copy — never checked in.
