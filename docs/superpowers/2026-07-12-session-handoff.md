...
- **⭐ TODO (requested 2026-07-12): full Nastaliq-aware kashida in apply.** Right now `applyProfileToQaseeda`'s justify pass only does **generic tatweels** for `generic`/`tatweel` fonts and **spacing** for everything else (the `7ee982f` guard). It does NOT do the *proper* per-font kashida that `justifySelection` does. Port those per-cell mechanism branches into the apply justify loop (ideally factor a shared `justifyCell(...)` helper both paths call):
  - **Jameel `font-swap`** — `justifySelectionInner` ~L2213–2255: `AshaarKashidaFontswap.splitSpans` → measure each fasl in base (`wordName`) vs wide (`kasheedaName`) face → `selectSwapRuns` to a target → rebuild per-run OOXML via `AshaarWord.runsToMisraXml` (natural-fit) or `misraDistributeXml` (cell-fit); capped hair-space residual. Requires force-loading both faces before measuring (see [[font-measurement-model]]; ~L2150–2160).
  - **Mehr `tatweel`** — ~L2263–2302: discrete trailing-tatweel subset selection via `AshaarWord.mehrElongate(tok, isoSet, finSet)` (from `cellDesc.tatweelRules`) + `selectSwapRuns`; single-font text output + hair-space residual. (Currently Mehr wrongly runs the *generic* engine in apply — passable but not its real mechanism.)
  - **Noto/whitespace** — already correct (spacing).
  - Per-cell mechanism comes from `AshaarFonts.descriptorForFontName(cellFontName).mechanism` (apply already computes `fmech`). Reuse the natural-fit/cell-fit target math already in the apply loop.
  - Note: apply RENDERS bare then justifies text into fixed-width cells; `justifySelection` REBUILDS via `insertPoem`. The font-swap/Mehr branches only rewrite cell CONTENT (OOXML/text), so they slot into the apply justify pass without a rebuild — good.
- **Cell-fit distribute quirk carries over**: the font-swap/Mehr cell-fit branches in `justifySelection` use `misraDistributeXml` (Word `distribute`), which no-ops on single lines — when porting, prefer the micro-space fill (as the generic apply path now does) for visible results.
